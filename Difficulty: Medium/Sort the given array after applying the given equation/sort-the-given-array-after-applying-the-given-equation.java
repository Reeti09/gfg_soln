...
class Solution {
    public ArrayList<Integer> sortArray(int[] arr, int A, int B, int C) {
        ...
        // For choosing direction based on the parabola's opening
        int index = A >= 0 ? n - 1 : 0;

        while (i <= j) {
            int val1 = evaluate(arr[i], A, B, C);
            int val2 = evaluate(arr[j], A, B, C);
            if (A >= 0) {
                if (val1 > val2) {
                    res[index--] = val1;
                    i++;
        ...
        // Convert result array to ArrayList
        ArrayList<Integer> output = new ArrayList<>(n);
        for (int val : res) output.add(val);
        return output;
    }
